...
In base ai dati acquisiti, il tracker è in grado di eseguire una classificazione accurata, oltre a non avere problemi legati all' individuazione di falsi positivi. La matrice $\mathbf{A}$  permette di ottenere le migliori performance di classificazione, per un classificatore di tipo Nearest Neighbour (k-NN), nel nuovo spazio a $d$ dimensioni. Il classificatore esegue una predizione sulla classe di un dato in ingresso in basse alle informazioni sui k dati più vicini, in base a una metrica data. Questo metodo non è comunque differenziabile rispetto ad $\mathbf{A}$, per questo viene utilizzata una rappresentazione più semplice dei nearest neighbours. Per misurare la relazione di vicinanza tra due coppie di vettori $x_i$ e $x_j$ del training set, viene utilizzata una misura probabilistica $p_{ij}$ : 
$$p_{ij} = \frac{\text{exp}(-\| \mathbf{A}x_i - \mathbf{A}x_j  \| ^2)}{ \sum_{k \neq i} \text{exp}(-\| \mathbf{A}x_i - \mathbf{A}x_k  \| ^2)}$$.

Quindi ogni vettore di feature $x_i$ tratta un altro vettore di feature $x_j$ con probabilità $p_{ij}$. Possiamo quindi individuare la probabilità $p_i$ che il punto $i$ venga correttamente classificato :
