...
U = get_U(subIm, m);
%K = get_K(subIm); %gi� normalizzato, pixel centrati e la relativa distanza dal centro
K = T.target.K;
...
%         %una riga in pi� rispetto al target -> togliamo una riga
...
%          %mettiamo tutti zeri per semplicit�
%          subIm = [ subIm; zeros(1, size(subIm,2), 3) ];
%         end
%     end
%     if(size(subIm,2) > size(T.target.subIm,2))
%         %una colonna in pi� rispetto al target -> togliamo una colonna
